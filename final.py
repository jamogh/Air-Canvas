    ...
    if result.multi_hand_landmarks:
        ...
        if (thumb[1] - center[1] < 30):
            ...
            if colorIndex == 0:
                ...
                cv2.circle(paintWindow, (center[0], center[1]),3, colors[colorIndex], -1)
            elif colorIndex == 1:
        ...
        # Draw the points on the paint window for each color
        for i in range(len(bpoints)):
            for j in range(len(bpoints[i])):
                cv2.circle(paintWindow, (bpoints[i][j][0], bpoints[i][j][1]), 3, colors[0], -1)
        for i in range(len(gpoints)):
            for j in range(len(gpoints[i])):
                cv2.circle(paintWindow, (gpoints[i][j][0], gpoints[i][j][1]), 3, colors[1], -1)
        for i in range(len(rpoints)):
            for j in range(len(rpoints[i])):
                cv2.circle(paintWindow, (rpoints[i][j][0], rpoints[i][j][1]), 3, colors[2], -1)
        for i in range(len(ypoints)):
            for j in range(len(ypoints[i])):
                cv2.circle(paintWindow, (ypoints[i][j][0], ypoints[i][j][1]), 3, colors[3], -1)

    # Show the paint window and webcam feed
